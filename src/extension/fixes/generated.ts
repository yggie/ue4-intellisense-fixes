...
const GLOB_UE_EDITOR_GENERATED =
  "Intermediate/Build/**/UnrealEditor/Inc/**/*.generated.h";
const GLOB_UE4_EDITOR_GENERATED =
  "Intermediate/Build/**/UE4Editor/Inc/**/*.generated.h";

const REGEX_UE_EDITOR_GENERATED =
  /(?<!Engine)[\/\\]Intermediate[\/\\]Build[\/\\](?:\w+[\/\\]){1,2}(?:Unreal|UE4)Editor[\/\\]Inc/gm;
...
export async function fixGenerated(project: ProjectUE4): Promise<void> {
  ...
  const relPatternUEEditor = new vscode.RelativePattern(
    mainWorkspace,
    GLOB_UE_EDITOR_GENERATED
  );
  const relPatternUE4Editor = new vscode.RelativePattern(
    mainWorkspace,
    GLOB_UE4_EDITOR_GENERATED
  );
  ...
  const uniqueResponsePaths =
    getUniqueResponsePathsFromCompileCommands(compileCommands);
  if (
    await hasRegexInAllFiles(REGEX_UE_EDITOR_GENERATED, uniqueResponsePaths)
  ) {
...
function getUniqueResponsePathsFromCompileCommands(
  compileCommands: Map<string, CompileCommands>[]
) {
...
async function hasRegexInAllFiles(
  regExp: RegExp,
  paths: string[]
): Promise<boolean> {
  for (const path of paths) {
    ...
    if (!match?.length) {
      console.error(
        `No match found for RegExp(${regExp.source}) on file: ${path}`
      );
      console.error(
        "You must refresh your project by Generating Project files!"
      );
      console.error(
        "You can do this with VSCode->Run/Debug(MS C# Ext required), right-clicking project's *.uproject file, or in the Unreal Engine menus."
      );
      return false;
    }
  }
