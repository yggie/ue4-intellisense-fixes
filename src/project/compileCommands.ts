...
export class CompileCommands {
  ...
  private constructor(path: string, compileCommands: CommandObjectJson[]) {
    ...
    if (this.length) {
      // This class is an iterator so it checks _compileCommands length
      this._isValid = true;
    }
  }
  ...
  *[Symbol.iterator]() {
   ...
   */
  protected static async createCompileCommands(
    path: string
  ): Promise<CommandObjectJson[]> {
  ...
  getUniqueMatchesFromAllCommandLines(regExp: RegExp): string[] | undefined {
    if (!this._compileCommands[0]?.command) {
      console.error(
        "No compile commands found in compile command's file. Try resetting UE project."
      );
      return;
    }
    ...
    for (const commandObject of this) {
      ...
      const match: RegExpMatchArray | null =
        commandObject.command.match(regExp);
    ...
    if (!uniqueMatches.size) {
      console.log(
        `Regex couldn't parse matches in compile commands file. re: ${regExp.source} , flags: ${regExp.flags}`
      );
      return;
    }
  ...
  public async saveToFile() {
    ...
    const compileCommandsString = JSON.stringify(
